...
public class PersonPartDisplayDriver : ContentPartDisplayDriver<PersonPart>
{
    ...
    // - Also, some overrides will take a context object: When in doubt use the one with the context object, otherwise
    // there is no difference apart from what you can see: The context can be passed to helper methods and is otherwise
    // used in the background to carry some other contextual object like the current IUpdateModel. If you end up not
    // using the context anywhere in the method's body than that means that you don't actually need it, no magic will
    // happen behind the scenes because of it.
    ...
    public override IDisplayResult Display(PersonPart part, BuildPartDisplayContext context) =>
        ...
        // generate the view model (like fetching data from the database or an external
        ...
        Initialize<PersonPartViewModel>(GetDisplayShapeType(context), viewModel => PopulateViewModel(part, viewModel))
            // Note that again we're referring to a display type here just as with Books, the Detail display type. While
            // display types can be customized, by default Orchard uses "Detail" when the content item is opened in its
            // entirety on the frontend, "Summary" when it's listed on the frontend, and "SummaryAdmin" when it's listed
            // on the admin site. You can specify how a content part is displayed in these scenarios with these location
            // settings. In this simple case, the same shape will be used both in Detail and Summary.
    ...
    public override IDisplayResult Edit(PersonPart part, BuildPartEditorContext context) =>
        ...
        // Notice that there is no location given for this shape (no Location() method call). There's another option of
        // giving these locations using Placement.json files. Since it is not possible to put comments in a .json file
        // the explanation is here but make sure you check the file while reading this. It's important to give a
        // location somewhere otherwise to shape won't be displayed. The shape file should be in the Views folder by
        // default, however, it could be outside the Views folder too (e.g. inside the Drivers folder).

        // In Placement files you can give specific locations to any shapes generated by Orchard. You can also specify
        // rules to match when the location will be applied: like only for certain fields, content types, just under a
        // given path. In our Placement file you can see that the PersonPart shape gets the first position in the
        // Content zone and the TextField shape gets the second. To make sure that not all the TextFields will get the
        // same position a "differentiator" property is given which refers to the part name where the field is attached
        // to and the field name. Make sure you also read the documentation to know this feature better:
