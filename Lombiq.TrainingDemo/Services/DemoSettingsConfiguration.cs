...
public class DemoSettingsConfiguration : IConfigureOptions<DemoSettings>
{
    ...
    public void Configure(DemoSettings options)
    {
        ...
        // or will be null. You can decide what should happen if both settings are set. Here we'll use the other one if
        // it's set.
        if (!string.IsNullOrEmpty(options.Message)) return;

        // If you'd like to try this out, add an "OrchardCore" section to your web app's appsettings.json file. So in
        ...
        // Not that for this to work IShellConfiguration needs to be accessed from the Startup class, so quickly check
