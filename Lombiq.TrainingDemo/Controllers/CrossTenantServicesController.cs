...
public class CrossTenantServicesController : Controller
{
    ...
    public async Task<string> Index(string contentItemId)
    {
        ...
        // In this example, we'll access content items from the Default tenant but this works for any tenant of course.
        // Create a tenant in your app (enable the Tenants feature and then create it from under Configuration /
        // Tenants), enable the Training Demo on it too and check out how this works there!
        ...
        // With UsingAsync() we'll create a block where everything is executed within the context of that other tenant.
        // It's a bit similar to being inside a controller action, but remember that all of this is running on the
        // Default tenant, even if you're looking at it from another tenant you've created.
        await shellScope.UsingAsync(async scope =>
        {
            // You can resolve any service from the shell's Service Provider. This serves instead of injecting services
            // in the constructor.
            var contentManager = scope.ServiceProvider.GetRequiredService<IContentManager>();

            // We can use IContentManager as usual, it'll just work. Note that for the sake of simplicity there is no
