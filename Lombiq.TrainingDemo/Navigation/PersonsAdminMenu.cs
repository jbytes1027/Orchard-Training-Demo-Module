...
public class PersonsAdminMenu : INavigationProvider
{
    ...
    public ValueTask BuildNavigationAsync(string name, NavigationBuilder builder)
    {
        ...
        // click on this. It should have a position parameter as well which not surprisingly will be the position in the
        // admin menu.
        builder.Add(T["Person Pages"], "5", menu => menu
            ...
            // used if you want to override the menu item shape (because these are shapes too) in order to add a nice
            // looking icon to it. If you want to override it then use the NavigationItemText-[id].Id alternate when
            // creating the cshtml file.

            // NEXT STATION: Go check how it's done in Views/NavigationItemText-persons.Id.cshtml and come back here!
            ...
            // Now that is conventional to do, however, it really depends on the Admin theme. The TheAdmin theme will
            // never link it to the first child, it will always be a dropdown without a link.
            .LinkToFirstChild(value: true)
            // Now let's add the sub menu items with the same Add() method we used for the first-level item but chained
            // to this one.
            .Add(T["Test"], subitem => subitem
                // The Action method will bind the menu item to the action. This is the test action that we've added to
                // the AdminController to see if this is working automatically. Note the use of Orchard's
            ...
            // Add another menu item that will display multiple Person items. However, branch this item to two different
            // third-level items!
            .Add(T["Person List"], subitem => subitem
                ...
                .Add(T["Newest Items"], thirdLevelItem => thirdLevelItem
                    .Action(
                        ...
                        typeof(AdminController).ControllerName(),
                        new { area = $"{nameof(Lombiq)}.{nameof(TrainingDemo)}" })
                    // This means that the first child menu item will point to our Person List dashboard and be shown
                    // only to users having the AccessPersonListDashboard permission. WARNING: this doesn't mean others
                    // won't be able to access it directly: we have to check in the controller too!
                    ...
                    // menu. Most probably it will only happen if the theme doesn't support the third-level menu items
                    // and it will display them somewhere else. In the TheAdmin theme it won't make any difference since
                    // the three-level menu is supported.
                    .LocalNav())
